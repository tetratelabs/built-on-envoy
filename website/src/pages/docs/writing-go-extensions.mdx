...
The `composerVersion` field must match the version of the Composer dynamic module that will load your plugin.
...
Dynamic Modules have direct access to the Envoy memory buffers. While this allows a zero-copy model when invoking extension methods,
buffers need to be accessed carefully. To this end, the SDK wraps the data in a `shared.UnsafeEnvoyBuffer` struct, and callers have to
explicitly call the right methods with explicit intent on copying or directly accessing the data.

```go
// Get direct access to the Envoy memory buffers (zero-copy)
buffer.ToUnsafeBytes()
buffer.ToUnsafeString()

// Get a copy of the data for safe access
buffer.ToBytes()
buffer.ToString()
```
...
func (f *customHttpFilter) OnRequestBody(body shared.BodyBuffer, endStream bool) shared.BodyStatus {
    ...
    var original []shared.UnsafeEnvoyBuffer
    for _, chunk := range bufferedBody.GetChunks() {
        original = append(original, chunk.ToUnsafeBytes()...)
    }
...
```go
func (f *customHttpFilter) OnRequestHeaders(headers shared.HeaderMap, endStream bool) shared.HeadersStatus {
    if headers.GetOne("x-block").ToUnsafeString() == "true" {
...
func (f *customHttpFilter) OnRequestHeaders(headers shared.HeaderMap, endStream bool) shared.HeadersStatus {
    ...
    f.handle.Log(shared.LogLevelInfo, "Request host: %s", host.ToUnsafeString())
    return shared.HeadersStatusContinue
}
```
