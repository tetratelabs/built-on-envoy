...
[↑ Top](#) | [↑ example-wasm-go Configuration](#example-wasm-go) {/* if .SubProperties */} {/* range .Properties */} {/* if .CustomTypes */}
...
Configuration for the web-terminal extension. Streams a PTY to the browser over SSE and takes input via POST. A browser terminal is remote code execution by design: set a token unless the listener is otherwise protected.
...
[↑ Top](#) | [↑ Web Terminal Configuration](#web-terminal) {/* if .SubProperties */}

### command

Executable to run inside the PTY (e.g. "/bin/bash").
...
Whether to serve the bundled xterm.js client at "/". Disabled by default; the /stream, /input and /resize endpoints are the primary interface. Set to true to serve the bundled client.
...
Whether client keystrokes are written to the PTY. Set to false for a read-only terminal.
...
[↑ Top](#) | [↑ Web Terminal Configuration](#web-terminal) {/* if .SubProperties */} {/* range .Properties */} {/* if .CustomTypes */} {/* range . */}
