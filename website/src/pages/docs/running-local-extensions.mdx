...
- **composer** - A Go HTTP filter loaded at runtime by the Composer dynamic module (default).
- **dynamic_module_rust** - A Rust HTTP filter compiled as a native dynamic module.

### Creating a Go (Composer) Extension

```bash
boe create my-extension --path ~/src/extensions
```

This is equivalent to explicitly passing `--type composer`. The generated project structure looks like this:
