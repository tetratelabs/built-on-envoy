...
Rust extensions are compiled as native dynamic modules that Envoy loads directly, with no intermediate runtime or plugin loader.
They support multiple filter types — from HTTP request/response processing (L7) to raw TCP/TLS streams (L4) and UDP — each targeting a different Envoy layer with its own traits and lifecycle.
...
```bash
boe create my-extension --type rust --path my-extension
```
...
> **Tip:** The `filterType` field defaults to `http`. See [Filter Types](#filter-types) for all supported values and their templates.
...
The `filterType` field in your manifest determines which Envoy filter stage your extension targets.
It defaults to `http` if not specified.
...
> **Note:** The `nativeHttpFilters` manifest field is only available for the `http` filter type.
> If you need both HTTP and network filters in a single module, use `declare_all_init_functions!` instead.

### HTTP Filter (`filterType: http`)

The HTTP filter type is the default and most commonly used. It runs at the L7 stage and can inspect and modify request and response headers, bodies, and trailers for each HTTP stream.
...
### Network Filter (`filterType: network`)

Network filters run at the TCP/TLS (L4) stage, before HTTP decoding. They process raw byte streams, making them a good fit for custom protocol handling or connection-level access policies.

#### Filter Lifecycle

```mermaid
flowchart LR
    on_new_connection --> on_read & on_write & on_event --> on_destroy
```
...
impl<ENF: EnvoyNetworkFilter> NetworkFilterConfig<ENF> for FilterConfig {
...
fn new_network_filter_config_fn<EC: EnvoyNetworkFilterConfig, ENF: EnvoyNetworkFilter>(
    ...
    match filter_name {
        "my-network-extension" => Some(Box::new(FilterConfig)),
        _ => panic!("Unknown filter name: {filter_name}"),
    }
}
...
### Listener Filter (`filterType: listener`)

Listener filters run before a TCP connection is routed to a filter chain. They are useful for connection acceptance policies, pre-routing inspection, and setting connection metadata that downstream filters can read.

#### Filter Lifecycle

```mermaid
flowchart LR
    on_accept --> on_data --> on_close
```
...
fn new_listener_filter_config_fn<EC: EnvoyListenerFilterConfig, ELF: EnvoyListenerFilter>(
    ...
    match filter_name {
        "my-listener-extension" => Some(Box::new(FilterConfig)),
        _ => panic!("Unknown filter name: {filter_name}"),
    }
}
...
### UDP Listener Filter (`filterType: udp_listener`)

UDP listener filters process UDP datagrams at the listener level. They expose a single `on_data` callback called for each received datagram.
...
>(
    ...
    match filter_name {
        "my-udp-extension" => Some(Box::new(FilterConfig)),
        _ => panic!("Unknown filter name: {filter_name}"),
    }
}
...
> The following examples apply to the `http` filter type and its `EnvoyHttpFilter` API.
> Network, listener, and UDP listener filters expose their own APIs through the
> `EnvoyNetworkFilter`, `EnvoyListenerFilter`, and `EnvoyUdpListenerFilter` traits respectively.
