...
The `[lib] name` must use underscores (Rust convention), even if the extension name uses hyphens.
...
<Callout type="note">
The `filterType` field defaults to `http`. See [Filter Types](#filter-types) for all supported values and their templates.
</Callout>
...
<Callout type="warning">
The `nativeHttpFilters` manifest field is only available for the `http` filter type.
</Callout>
