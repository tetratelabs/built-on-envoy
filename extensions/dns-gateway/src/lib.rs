...
// Built On Envoy emits each generated filter with `FilterName` set to the manifest `name`
// ("dns-gateway"). Both the network and UDP listener filters therefore arrive under that same
// name, but they're dispatched by separate per-filter-type functions, so matching the manifest
// name in each is unambiguous: this function only ever handles the network (cache_lookup) filter.
...
>(
    ...
    // See the note on `new_network_filter_config_fn`: the filter arrives under the manifest name
    // ("dns-gateway"); this function only ever handles the UDP listener (dns_gateway) filter.
    match filter_name {
        "dns-gateway" => {
