...
impl DnsGatewayFilterConfig {
    ...
    ///
    /// Built On Envoy delivers the `--config` JSON string to the module verbatim, so the bytes are
    /// the gateway config directly: `{"domains":[...]}`.
    pub fn new(config: &[u8]) -> Option<Self> {
        let gateway_config: config::DnsGateway = std::str::from_utf8(config)
            ...
            .and_then(|s| {
                serde_json::from_str::<config::DnsGateway>(s)
