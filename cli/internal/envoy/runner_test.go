...
func TestSetupDynamicModuleSearchPath(t *testing.T) {
	...
	legacyGoPlugin := &extensions.Manifest{
		Name: "example-go", Type: extensions.TypeGo, Version: "0.1.0", ComposerVersion: composerVersion,
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			dirs := newDirs(t)
			params := &ConfigGenerationParams{Logger: internaltesting.NewTLogger(t), Dirs: dirs, Extensions: tt.exts}
