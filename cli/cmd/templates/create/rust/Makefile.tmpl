...
# Labels for this extension image.
NAME             := $(shell grep "^name:" manifest.yaml | sed 's/[^:]*:[[:space:]]*//g' | tr -d '"')
VERSION          := $(shell grep "^version:" manifest.yaml | sed 's/[^:]*:[[:space:]]*//g' | tr -d '"')
TYPE             := $(shell grep "^type:" manifest.yaml | sed 's/[^:]*:[[:space:]]*//g' | tr -d '"')
FILTER_TYPE      := $(shell grep "^filterType:" manifest.yaml | sed 's/[^:]*:[[:space:]]*//g' | tr -d '"')
...
# Convert lib name (hyphens to underscores for Rust)
LIB_NAME := $(shell echo "$(NAME)" | tr '-' '_')
...
.PHONY: test-coverage
test-coverage:
	cargo llvm-cov --lcov --output-path lcov.info --fail-under-lines $(COVERAGE_THRESHOLD); \
		status=$$?; \
...
.PHONY: build_image
build_image: create_builder  ## Build docker image for the extension for local platform
	@echo "Building image for local platform ($(OS)/$(ARCH))..."
	docker buildx build \
...
.PHONY: push_image
push_image: create_builder  ## Build and push docker image for the extension for cross-platform support
