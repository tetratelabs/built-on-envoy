...
func TestSetupGoPluginEnv(t *testing.T) {
	t.Run("no credentials sets cache dir only", func(t *testing.T) {
		dirs := &xdg.Directories{DataHome: t.TempDir()}
		cleanup, err := setupGoPluginEnv(OCIFlags{Registry: "ghcr.io/test"}, dirs)
		require.NoError(t, err)
		defer cleanup()

		require.Equal(t, filepath.Join(dirs.DataHome, "goplugin-cache"), os.Getenv("GOPLUGIN_CACHE_DIR"))
		require.Empty(t, os.Getenv("GOPLUGIN_PULL_SECRET"))
		require.Empty(t, os.Getenv("GOPLUGIN_INSECURE"))
	})

	t.Run("insecure sets GOPLUGIN_INSECURE", func(t *testing.T) {
		dirs := &xdg.Directories{DataHome: t.TempDir()}
		cleanup, err := setupGoPluginEnv(OCIFlags{Registry: "localhost:5000/test", Insecure: true}, dirs)
		require.NoError(t, err)
		defer cleanup()

		require.Equal(t, "true", os.Getenv("GOPLUGIN_INSECURE"))
	})

	t.Run("credentials writes docker config JSON", func(t *testing.T) {
		dirs := &xdg.Directories{DataHome: t.TempDir()}
		cleanup, err := setupGoPluginEnv(OCIFlags{
			Registry: "ghcr.io/tetratelabs/built-on-envoy",
			Username: "user",
			Password: "pass",
		}, dirs)
		...
		secretPath := os.Getenv("GOPLUGIN_PULL_SECRET")
		require.NotEmpty(t, secretPath)

		data, err := os.ReadFile(secretPath) //nolint:gosec
		require.NoError(t, err)
		require.Contains(t, string(data), `"ghcr.io"`)
		require.Contains(t, string(data), `"username":"user"`)
		require.Contains(t, string(data), `"password":"pass"`)
	})
